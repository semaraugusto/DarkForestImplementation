...
contract GameState {
    ...
    IVerifier spawn_verifier;
    IVerifier move_verifier;
    ...
    constructor(address _spawn_verifier_addr, address _move_verifier_addr, uint32 _min_distance, uint32 _max_distance) {
        ...
        spawn_verifier = IVerifier(_spawn_verifier_addr);
        move_verifier = IVerifier(_move_verifier_addr);
    }

    function _initializePlanet(uint planet_type, Types.Planet storage planet) internal returns(bool) {
        emit Log("Planet type", planet_type);
        require(planet_type != 0, "Not a planet!");
        planet.initialized = true;
        if(planet_type == 1) {
            planet.remaining_resources = 1000;
            planet.resources_per_turn = 10;
        } else if(planet_type == 2) {
            planet.remaining_resources = 10000;
            planet.resources_per_turn = 50;
        } else if(planet_type == 3) {
            planet.remaining_resources = 100000;
            planet.resources_per_turn = 150;
        } else {
            return false;
        }
        emit LogPlanet("Planet...", planet);
        return true;
    }
    ...
    function move(
        ...
        if (_input[0] == player.curr_pos) {
            ...
            // leaving planet
            require(planet_from.num_players_occupying >= 1, "Leaving empty planet");
    ...
    function getPlanets(uint addr) public view returns(Types.Planet memory) {
        return planets[addr];
    }
